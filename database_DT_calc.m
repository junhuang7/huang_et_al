...
timevec1 = 0:(1/SR):((length(DT_task.dFF0{j})/SR)-1/SR);
timevec_RT=-prestart_RT:1/SR:poststart_RT;
%% calculate signal
% preallocation of array, which saves 0.2 seconds in one session

TraceTrl = zeros(length(DT_task.Trial_Onsets{j}),6*SR+1); 
lTraceTrl = zeros(length(DT_task.Trial_Onsets{j}),6*SR+1);

for i=1:length(DT_task.Trial_Onsets{j})-1;
    TraceTrl(i,:)=DT_task.dFF0{j}(DT_task.Trial_Onsets{j}(i)*SR-prestart*SR:DT_task.Trial_Onsets{j}(i)*SR+poststart*SR);
end

for i=1:length(DT_task.Trial_Onsets{j})-1;
    lTraceTrl(i,:)=DT_task.lick{j}(DT_task.Trial_Onsets{j}(i)*SR-prestart*SR:DT_task.Trial_Onsets{j}(i)*SR+poststart*SR);
end

TraceTrl_RT = zeros(length(DT_task.Trial_Onsets{j}),6*SR+1); 
lTraceTrl_RT = zeros(length(DT_task.Trial_Onsets{j}),6*SR+1);

for i=1:length(DT_task.Trial_Onsets{j})-1;
    TraceTrl_RT(i,:)=DT_task.dFF0{j}(DT_task.Trial_Onsets{j}(i)*SR-prestart_RT*SR+DT_task.RT{j}(i):DT_task.Trial_Onsets{j}(i)*SR+poststart_RT*SR+DT_task.RT{j}(i));
end

for i=1:length(DT_task.Trial_Onsets{j})-1;
    lTraceTrl_RT(i,:)=DT_task.lick{j}(DT_task.Trial_Onsets{j}(i)*SR-prestart_RT*SR+DT_task.RT{j}(i):DT_task.Trial_Onsets{j}(i)*SR+poststart_RT*SR+DT_task.RT{j}(i));
end

%% define trial type

if size((DT_task.behavior_data{j}),2) ==30;
    hit=find(DT_task.behavior_data{j}(:,6)==1&DT_task.behavior_data{j}(:,8)==1&DT_task.behavior_data{j}(:,16)==0);
    miss=find(DT_task.behavior_data{j}(:,6)==1&DT_task.behavior_data{j}(:,8)==0&DT_task.behavior_data{j}(:,16)==0);
    CR=find(DT_task.behavior_data{j}(:,6)==0&DT_task.behavior_data{j}(:,8)==0&DT_task.behavior_data{j}(:,16)==0);
    FA=find(DT_task.behavior_data{j}(:,6)==0&DT_task.behavior_data{j}(:,8)==1&DT_task.behavior_data{j}(:,16)==0);
    EL=find(DT_task.behavior_data{j}(:,16)==1);
elseif size((DT_task.behavior_data{j}),2) ==18;
        hit=find(DT_task.behavior_data{j}(:,6)==1&DT_task.behavior_data{j}(:,9)==1);
        miss=find(DT_task.behavior_data{j}(:,6)==1&DT_task.behavior_data{j}(:,9)==0);
        CR=find(DT_task.behavior_data{j}(:,6)==0&DT_task.behavior_data{j}(:,9)==0);
        FA=find(DT_task.behavior_data{j}(:,6)==0&DT_task.behavior_data{j}(:,9)==1);
        EL=find(isnan(DT_task.behavior_data{j}(:,9)));
end
